...
public class NavNodeEditor : MonoBehaviour
{
	...
	private void OnDrawGizmos()
	{
		...
		{
			Gizmos.DrawIcon(navNode.transform.position + Vector3.up, "Node.png", true, Color.green);
			Gizmos.color = Color.green;
			Gizmos.DrawWireSphere(navNode.gameObject.transform.position, navNode.radius);
		}
		if (activeNavNode != null)
		{
			Gizmos.DrawIcon(activeNavNode.transform.position + Vector3.up, "Node.png", true, Color.red);
